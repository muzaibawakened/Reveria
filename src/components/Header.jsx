...
export default function Header({ currentView, onNavigate, onBackup, onRestore, dreamCount }) {
  ...
  const handleRestore = async (e) => {
    ...
    try {
      ...
      setRestoreMsg(count === 0 ? "No new dreams found — already up to date." : `${count} dream${count !== 1 ? "s" : ""} restored!`);
    } catch (err) {
      setRestoreStatus("error");
      setRestoreMsg(err.message || "Restore failed. Check the file format.");
    } finally {
  ...
  return (
    <>
      <motion.header
        ...
        className="fixed top-0 left-0 right-0 z-40 px-5 py-3 flex items-center"
      >
        {/* Logo */}
        <div
      ...
      <AnimatePresence>
        {menuOpen && (
          <motion.div
            ...
            className="fixed top-14 right-4 z-50 w-56 rounded-2xl overflow-hidden"
            style={{
              background: "linear-gradient(135deg, rgba(19,19,48,0.97) 0%, rgba(12,12,29,0.97) 100%)",
          ...
          >
            ...
            <div className="p-2">
              <MenuButton
                ...
                onClick={() => navTo("record")}
              />
              <MenuButton
                icon={<BookOpen className="w-3.5 h-3.5" />}
                label={dreamCount > 0 ? `Dream Vault (${dreamCount})` : "Dream Vault"}
            ...
            {/* Divider */}
            <div style={{ height: 1, background: "rgba(201,169,110,0.08)", margin: "0 12px" }} />
            ...
            <div className="p-2">
              <MenuButton
                ...
                label="Backup Dreams"
                onClick={() => { onBackup(); setMenuOpen(false); }}
                disabled={dreamCount === 0}
                hint={dreamCount === 0 ? "No dreams yet" : `${dreamCount} dream${dreamCount !== 1 ? "s" : ""}`}
              />
              <MenuButton
      ...
      <AnimatePresence>
        {restoreStatus && (
          <motion.div
            ...
            style={{
              background: restoreStatus === "error"
                ? "rgba(180,50,50,0.9)"
                : "rgba(19,19,48,0.95)",
              border: `1px solid ${restoreStatus === "success" ? "rgba(124,106,239,0.4)" : restoreStatus === "error" ? "rgba(255,100,100,0.3)" : "rgba(201,169,110,0.2)"}`,
          ...
          >
            {restoreStatus === "loading" && (
              <motion.div className="w-3 h-3 rounded-full bg-gold" animate={{ opacity: [0.4, 1, 0.4] }} transition={{ duration: 1, repeat: Infinity }} />
            )}
            {restoreStatus === "success" && <Check className="w-3.5 h-3.5 text-violet flex-shrink-0" />}
            {restoreStatus === "error" && <AlertCircle className="w-3.5 h-3.5 text-red-300 flex-shrink-0" />}
            <span className="text-xs font-ui tracking-wide text-parchment-mid">{restoreMsg}</span>
          </motion.div>
        )}
      </AnimatePresence>
    </>
...
function MenuButton({ icon, label, onClick, active, disabled, hint }) {
  return (
    <button
      ...
      className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-xl text-left text-xs font-ui tracking-wide transition-all duration-200 ${
        active
          ...
          : disabled
          ? "text-text-ghost cursor-default opacity-50"
          : "text-text-secondary hover:bg-white/5 hover:text-parchment"
      }`}
      style={{ minHeight: 40 }}
    >
      <span className={active ? "text-gold" : "text-text-ghost"}>{icon}</span>
